...
func newSubmitLifecycleFixture(t *testing.T, model string, effort *string) (*SubmitTaskUseCase, *submitLifecycleAdmitter, *store.FileTaskStore, string) {
	...
	starter := &submitSynchronousStarter{start: func(payload execution.TaskLaunchPayload) {
		...
		if payload.WorkingDir == nil {
			t.Fatal("working dir was not resolved")
		}
		if err := recordStarting.Execute(context.Background(), payload.Task, payload.ResolvedTimeout, payload.Model, payload.ReasoningEffort, payload.SandboxMode, *payload.WorkingDir, domain.ExecutionRouteDaemon, payload.PromptText, clock.Now()); err != nil {
			t.Fatal(err)
		}
	}}
