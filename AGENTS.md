...
- **技術選定・アーキテクチャを変更しない。** Go の採用、常駐プロセス方式、擬似端末の割り当て、
  ファイルロックによる生死判定、出力契約の凍結は、いずれも `01-architecture/adr/` が定める
  不可逆な決定である。変更が必要だと思えた場合は、まず Issue で論点を共有すること（
  `CONTRIBUTING.md` の手順に従う）。
